...
export function useExportDepartmentProcedureData(
  departmentId: string,
  dialogOpen: boolean,
) {
  const { data, isLoading } =
    trpc.procedures.listForExportByDepartment.useQuery(
      { departmentId },
      { enabled: dialogOpen },
    );
  ...
  const exportWithFormat = useCallback(
    (format: ExportFormat) => {
      ...
      const createdAt = new Date()
        ...
        .slice(0, 19)
        .replace(/:/g, "-");
      const baseName = `department-procedures-export-${createdAt}`;
